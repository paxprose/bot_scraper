...
(async () => {
    try {
        ...
        if(config.nvidia.active) { endpoints.push(new Nvidia(config.nvidia)); }
        if(config.bestbuy.active) { endpoints.push(new BestBuy(config.bestbuy)); }
        if(config.amazon.active) { endpoints.push(new Amazon(config.amazon)); }
        while(!cancel) {    
